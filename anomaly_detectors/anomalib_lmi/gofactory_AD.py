...
def predict(
    ...
    model = AnomalyModel2(
        ...
        tile=tile,
        stride=stride,
        tile_mode="resize" if resize else "padding",
    )
    ...
    for path_src, anom in zip(path_all, anom_all):
        ...
        if annotate:
            annotated_image = model.annotate(
                cv2.imread(path_src),
                ad_scores=anom_map,
                ad_threshold=anom_map.min(),
                ad_max=anom_map.max(),
            )
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        type=int,
        nargs="*",
        help="tile hight and width. Can be a single int or two integers",
    )
    ap.add_argument(
        "--stride",
        type=int,
        nargs="*",
        help="stride hight and width. Can be a single int or two integers",
    )
    ap.add_argument(
        "--resize",
        action="store_true",
        help="interpolate if it needs to resize images, otherwise pad zeros",
    )
    ap.add_argument(
        "--annotate",
        action="store_true",
