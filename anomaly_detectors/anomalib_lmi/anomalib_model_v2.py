...
class AnomalyModel_V2(Anomalib_Base):
    ...
    def __init__(self, model_path, tile=None, stride=None, tile_mode="padding", **kwargs):
        ...
        if ext == ".engine":
            ...
            try:
                ...
                for d in self.pt_model.pre_processor.transform.transforms:
                    if type(d).__name__ == "Resize":
                        self.model_shape = to_list(d.size)
                        self.image_size = to_list(d.size)
                        self.logger.info(f"Model Shape/Image Size: {self.model_shape}")
    ...
    def _infer(self, input_batch):
        ...
        if self.inference_mode == "TRT":
            ...
            self.logger.info(f"PT model output tensor shape: {output_tensor.shape} input shape: {input_batch.shape}")
