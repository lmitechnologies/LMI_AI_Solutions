...
class PaDiM(object):
    def __init__(self, GPU_memory=None):
        ...
        try:
            ...
            logging.warning(
                "Failed to import tensorflow_addons, will use scipy for gaussian filter. You should expect a performance penalty."
            )
    ...
    def embedding_net(self, net_type, shape=None, layer_defs={}):
        ...
        shape_embed = (
            layer1.shape[1],
            layer1.shape[2],
            layer1.shape[3] + layer2.shape[3] + layer3.shape[3],
        )
        shape_img = (h, w, ch)
    ...
    def padim_train(
        ...
        if err_ceil_z is not None:
            ...
            if is_plot:
                plot_fig(
                    predict_results,
                    self.training_mean_dist,
                    self.training_std_dist,
                    "./training_validation",
                    err_ceil_z=err_ceil_z,
                )
    ...
    def predict(self, dataset):
        ...
        for x, fname in dataset:
            ...
            if self.tfa_gaussian_filter2d:
                ...
                dist_tensor_x = self.scipy_gaussian_filter(
                    dist_tensor_x, sigma=1, radius=1
                )  # the size of the kernel along each axis will be 2*radius + 1
    ...
    def convert_tensorRT(self, saved_model_dir, trt_saved_model_dir, precision_mode="FP16"):
        ...
        max_workspace_size_bytes = (
            1073741824  # Default: 1e9. The maximum GPU temporary memory which the TensorRT engine can use at execution time
        )
        ...
        def get_trt_precision():
            if precision_mode == "FP32":
                ...
                raise RuntimeError("Unknown precision received: `{}`. Expected: FP32, FP16 or INT8")
