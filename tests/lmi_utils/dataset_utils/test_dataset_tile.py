...
def test_a_mask_with_separate_regions_reaches_only_the_tiles_its_pixels_touch():
    # 2x2 grid of 60px tiles on a 110x110 image padded to 120; regions in the top-left and bottom-right corners
    mask = np.zeros((110, 110), np.uint8)
    mask[5:15, 5:15] = 1
    mask[100:110, 95:110] = 1
    empty = MaskAnnotation("e", "defect", Mask(np.zeros((110, 110), np.uint8)))
    tiles = tile_annotated_image(_image(110, 110), [MaskAnnotation("m", "defect", Mask(mask)), empty], tile_size=60, stride=60)

    assert [len(t[3]) for t in tiles] == [1, 0, 0, 1]
    top_left = tiles[0][3][0].value.to_numpy(h=60, w=60)
    bottom_right = tiles[3][3][0].value.to_numpy(h=60, w=60)
    assert top_left[5:15, 5:15].all() and top_left.sum() == 100
    assert bottom_right[40:50, 35:50].all() and bottom_right.sum() == 150
