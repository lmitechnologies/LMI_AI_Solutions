...
def yolo_models():
    ...
    model_lists = [OD_DET_MODELS, OD_SEG_MODELS, OD_OBB_DOTA, OD_OBB_DOTA_8, OD_POSE_MODELS]
...
def yolo_models_api():
    ...
    for k,ml,task in zip(keys, model_lists, tasks):
        models[k] = [
            ObjectDetector(
                metadata=dict(version='v1', model_name='yolov8' if 'yolov8n' in model else 'yolov11', task=task, framework='ultralytics', model_path=model, image_size=[640, 640]),
                device=DEVICE
            ) for model in ml
        ]
    return models
