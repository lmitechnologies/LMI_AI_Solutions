...
def model_det():
    return [
        Yolov8_cls(model) for model in CLS_MODELS
    ]
...
def model_det_api():
    return [
        Classifier(metadata=dict(version='v0', model_name='yolov8', task='classification', framework='ultralytics', model_path=model, image_size=[MODEL_SZ, MODEL_SZ])) for model in CLS_MODELS
    ]
    ...
    def test_predict(self, model_det, imgs_coco):
        ...
        for model in model_det:
            for img,resized,op in zip(*imgs_coco):
                ...
                im_out = cv2.putText(img.copy(), label, (10,30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), 2)
