...
def load_model(configs):
    ...
    if operation == "convert" and configs.get("conversion_configs") == {}:
        raise ValueError("Conversion configuration is missing.")
    if model_type == "nano":
        if operation == "train":
            return RFDETRNano()
...
def main():
    ...
    if configs.get("model_configs").get("operation") == "convert":
        output_dir = configs.get("conversion_configs", {}).get(
            "output_dir", os.path.dirname(configs.get("conversion_configs", {}).get("pretrain_weights", ""))
        )
        model = load_model(configs)
