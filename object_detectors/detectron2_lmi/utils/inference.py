...
class Detectron2Model(ODBase):
    ...
    def warmup(self):
        ...
        input = dict(
            ...
            .to(torch.float32)
            .to(self.device),
            height=self.cfg.INPUT.MAX_SIZE_TEST,
            width=self.cfg.INPUT.MAX_SIZE_TEST,
        )
    ...
    def forward(self, input, **kwargs):
        ...
        # t0 = time.time()
        with torch.no_grad():
            outputs = self.model.inference([input], do_postprocess=kwargs.get("do_postprocess", False))
        # t1 = time.time()
        return outputs
    ...
    def postprocess(self, results, image_height, image_width, confs, return_segments=False, **kwargs):
        ...
        postprocessed_results["classes"] = (
            [self.class_map.get(int(label), str(label)) for label in classes[keep].cpu().numpy()] if classes is not None else np.array([])
        )
        ...
        if return_segments:
            ...
            postprocessed_results["segments"] = [
                GenericMask(x, image_height, image_width).polygons[0].reshape(-1, 2) for x in postprocessed_results["masks"]
            ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        help="Path to the confidence map file",
        default=0.5,
    )
    parser.add_argument(
        "--class-map",
        type=str,
        help="Class map json file",
        default="/home/class_map.json",
    )
    ...
    confidence_map = {str(v): args.confidence_threshold for k, v in class_map.items()}

    model = Detectron2Model(
        weights_path=args.weights,
        config_file=args.config_file,
        detectron2_config_file=args.detectron2_config,
        class_map=class_map,
    )
    ...
    write_to_csv(shapes, os.path.join(args.output_path, "predictions.csv"), overwrite=True)
