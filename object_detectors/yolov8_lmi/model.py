...
class Yolov8(ODBase):
    ...
    def postprocess(
        ...
        preds2 = nms.non_max_suppression(
            preds,
            conf2,
            iou,
            agnostic=agnostic,
        ...
        for i, pred in enumerate(preds2):  # pred2: [x1, y1, x2, y2, conf, cls, mask1, mask2 ...]
            ...
            if predict_mask:
                ...
                if return_segments:
                    segments = [ops.scale_coords(masks.shape[1:], x, orig_img.shape, normalize=False) for x in ops.masks2segments(masks)]
                    if return_tensor:
    ...
    def annotate_image(
        ...
        for i in range(len(boxes)):
            ...
            if boxes[i].shape == (4, 2):
                ...
                pipeline_utils.plot_one_box(boxes[i], image, masks[i] if len(masks) else None, **args)
