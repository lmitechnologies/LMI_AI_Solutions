...
tf.compat.v1.flags.DEFINE_string("inference_graph", None, "Path to the inference graph with embedded weights.")
tf.compat.v1.flags.DEFINE_string("class_labels", None, "Path to classes.pbtxt file.")
tf.compat.v1.flags.DEFINE_string("draw_option", "False", "Whether or not to save images with boxes drawn")
tf.compat.v1.flags.DEFINE_string("draw_save_path", None, "If using draw_option, where to save images")
...
class CustomParser(data_parser.DataToNumpyParser):
    ...
    def __init__(self):
        self.items_to_handlers = {
            fields.InputDataFields.image: StringParser(fields.TfExampleFields.image_encoded),
                ...
                for i in range(len(groundtruth_boxes)):
                    if matches.shape[0] > 0 and matches[matches[:, 0] == i].shape[0] == 1:
                        confusion_matrix[groundtruth_classes[i] - 1][
                            int(detection_classes[int(matches[matches[:, 0] == i, 1][0])] - 1)
                        ] += 1
                    else:
