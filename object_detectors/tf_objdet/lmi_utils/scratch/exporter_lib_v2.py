...
def _combine_side_inputs(side_input_shapes="", side_input_types="", side_input_names=""):
...
class DetectionInferenceModule(tf.Module):
    ...
    def _run_inference_on_images(self, image, **kwargs):
        ...
        Args:
          image: uint8 Tensor of shape [None, None, None, 3].
        ...
        detections[classes_field] = tf.cast(detections[classes_field], tf.float32) + label_id_offset
...
class DetectionFromImageModule(DetectionInferenceModule):
    ...
    def __init__(self, detection_model, use_side_inputs=False, zipped_side_inputs=None):
        ...
        if zipped_side_inputs is None:
            zipped_side_inputs = []
        sig = [tf.TensorSpec(shape=[None, None, None, 3], dtype=tf.uint8, name="input_tensor")]
        if use_side_inputs:
