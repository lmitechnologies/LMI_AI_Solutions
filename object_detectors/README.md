...
Note: `prompt_with_similarity2.py` and `prompt.py` are required to install the dependencies in virtual environment without docker. Check the dependencies in the `dockerfile`.

### 1.3 modify the docker-compose.yaml file

Modify `v2/docker-compose.yaml` file:
- Modify the path to input folder
- Copy the images to the input folder
- Replace the `class-0` with the real class name
- Modify the path to output folder
- (optional) modify configs and weights files
```yaml
services:
  sam2-pre-label:
    ...
    volumes:
      ...
      - ./configs:/app/configs
      - ./weights:/app/weights
      - ./outputs/sisu:/app/outputs
      - ./automatic_mask_generator.py:/app/automatic_mask_generator.py
      - ../my_utils.py:/app/my_utils.py
    command: >
      bash -c "python /app/automatic_mask_generator.py -i /app/data -o /app/outputs -c class-0"
```
