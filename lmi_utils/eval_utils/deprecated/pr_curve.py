    ...
    for k in dt_y:
        if k == "all":
            ax.plot(
                ...
                linewidth=3,
                color="blue",
                label=f"all classes @ iou_threshold={iou}",
            )
        else:
...
if __name__ == "__main__":
    ...
    parse.add_argument(
        ...
        "--path_out",
        required=True,
        help="the output path for storing Precision and Recall figures",
    )
    parse.add_argument(
        "--skip_classes",
        default="",
        help="skip calculating the P/R curves for these comma separated classes",
    )
    parse.add_argument("--iou", type=float, default=0.5, help="[optional] the iou threshold, default=0.5")
    parse.add_argument(
        "--image_level",
        action="store_true",
        help="[optional] calculate the precision and recall on image level",
    )
    ...
    plot_curve(
        X,
        Ps,
        save_dir=os.path.join(out_path, "precision" + postfix + ".png"),
        ylabel="Precision",
        iou=iou,
        step=0.05,
    )
    plot_curve(
        X,
        Rs,
        save_dir=os.path.join(out_path, "recall" + postfix + ".png"),
        ylabel="Recall",
        iou=iou,
        step=0.05,
    )
