...
def generate_image_name(image_name, args):
    if args["operation"] == "resize":
        ...
        out_name = (
            os.path.splitext(image_name)[0] + f"_{args['operation']}_{-1 * args['angle']}" + ".png"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Process images with operations: resize, pad, or rotate.")

    # Common arguments for all operations
    parser.add_argument("--path_imgs", "-i", required=True, help="Path to the input images.")
    parser.add_argument(
        "--path_json",
        default="labels.json",
        help='Optional JSON file corresponding to the images (default: "labels.json").',
    )
    parser.add_argument("--path_out_images", "-oi", required=True, help="Path to save output images.")
    parser.add_argument(
        "--path_out_json",
        "-of",
        default="labels.json",
    ...
    parser.add_argument("--bg", action="store_true", help="Save background images that have no labels.")
    parser.add_argument(
        "--warn_crop",
        action="store_true",
    ...
    dim_parser.add_argument("--width", type=int, required=False, default=None, help="Output image width.")
    dim_parser.add_argument("--height", type=int, required=False, default=None, help="Output image height.")

    # resize, pad, and rotate parsers
    resize_parser = subparsers.add_parser("resize", parents=[dim_parser], help="Resize images")
