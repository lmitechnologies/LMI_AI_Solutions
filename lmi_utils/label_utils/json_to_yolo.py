...
def get_args():
    ...
    ap.add_argument(
        "--path_train_json",
        default="labels.json",
        help="[optional] the path of a json file for train",
    )
    ap.add_argument(
        "--path_val_json",
        default="labels.json",
        help="[optional] the path of a json file for val",
    )
    ...
    ap.add_argument(
        ...
        "-cy",
        required=False,
        help="[optional] the path of a yolo dataset yaml file",
    )
    ap.add_argument(
        "--target_classes",
        default="all",
    ...
    ap.add_argument(
        "--bg",
        action="store_true",
        help="save images with no labels, where yolo models treat them as background",
    )
    ap.add_argument(
        "--merge_box",
        action="store_true",
...
def convert_to_yolo(args):
    ...
    write_txts(
        train_yolo_dataset["image_labels"],
        path_txts=path_txts_train,
        fnames=train_files,
        file_id_map=train_file_id_map,
    )
    ...
    if len(val_files) > 0 and use_train_for_val is False:
        write_txts(
            val_yolo_dataset["image_labels"],
            path_txts=path_txts_val,
            fnames=val_files,
            file_id_map=val_file_id_map,
        )
    ...
    with open(fname, "w") as outfile:
        json.dump({k: int(v) for k, v in train_yolo_dataset["class_map"].items()}, outfile)
    ...
    copy_images_in_folder(
        path_img=path_train_imgs,
        path_out=path_out_imgs_train,
        fnames=train_fnames,
        file_id_map=train_file_id_map,
    )
    ...
    if len(val_fnames) > 0 and use_train_for_val is False:
        copy_images_in_folder(
            path_img=path_val_imgs,
            path_out=path_out_imgs_val,
            fnames=val_fnames,
            file_id_map=val_file_id_map,
        )
