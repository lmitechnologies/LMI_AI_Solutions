...
if __name__ == "__main__":
    ...
    ap.add_argument(
        "--input_json",
        default="labels.json",
        help="[optinal] the input json file name in imgs_path",
    )
    ap.add_argument(
        "--output_json",
        default="labels_scretched.json",
        help="the output json file that contains the scretched labels, corresponds to imgs_path",
    )
    ap.add_argument(
        "--object_labels",
        required=True,
        help="Comma separated list of labels to scretch.",
    )

    ap.add_argument(
        "--distance_x",
        default="12",
        help="the maximum distance to the left or right edges of a vertex that can be set to edge.",
    )
    ap.add_argument(
        "--distance_y",
        default="0",
        help="the maximum distance to the top or bottom edges of a vertex that can be set to edge.",
    )
    ...
    ap.add_argument(
        ...
        action="store_true",
        dest="annotating",
        help="annotate to visualize the scretched labels",
    )
