...
def rect_gui(wName, crop, bbox, rect_gui_event, cmd_line_event, image_display):
    ...
    w = lrx - ulx
    h = lry - uly
    selectinwindow.init(roi, image, wName, imageWidth, imageHeight, ulx, uly, w, h)
    # set mouse callbacks
    ...
    for key in label_dict:
        ...
        for region in list(regions):
            ...
            if current_label == target_label:
                ...
                crop = render[
                    np.maximum(y0 - dy0, 0) : np.minimum(y0 + height + dy0, H),
                ...
                with ThreadPoolExecutor(max_workers=2) as executor:
                    ...
                    while True:
                        ...
                        if image_display["runtime_image"] is not None:
                            cv2.imshow(WINDOW_NAME, image_display["runtime_image"])
                            cv2.waitKey(4)
                    ...
                    if not quit:
                        ...
                        dx0 = x0 if (x0 - render_delta) < 0 else render_delta
                        bbox = np.rint(
                            (bbox_scale_out / window_width * Wc) - np.asarray((dx0, dy0, dx0, dy0)) + np.asarray((x0, y0, x0, y0))
                        ).astype(int)
                        (ulx, uly, lrx, lry) = bbox
