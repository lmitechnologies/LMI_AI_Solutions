...
def getContours(img, blur=(17, 17), threshold=(20, 150)):
...
def align_and_crop(image_file_path, output_dir_path):
    # import image
...
def tile_image(image_file_path, tile_dir_path, window_dimension, steps_per_window):
    ...
    for j in range(j_y-1):
        for i in range(i_x-1):
            ...
            fwin = tile_dir_path+'/' + \
                os.path.splitext(os.path.split(image_file_path)[1])[
                    0]+'_'+str(i)+'_'+str(j)+'.png'
