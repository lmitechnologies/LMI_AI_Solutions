...
def convert_greyscale_image_to_color(greyscale_img):
    ...
    greyscale_x = np.linspace(greyscale_img.min(), greyscale_img.max(), num=TWO_TO_TWENTYFORUTH_MINUS_ONE)
    greyscale_int = np.interp(greyscale_img, greyscale_x, greyscale_y).astype(np.int)
