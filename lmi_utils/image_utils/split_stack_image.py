...
def split_stack_images(
    input_path: str,
    output_path: str,
    num_split: int,
    stack_direct: str,
    keep_same_filename=False,
):
    """
    split the images horizontally into segments, vstack these segments
    arguments:
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        required=True,
        type=int,
        help="the num of evenly splits horizontally",
    )
    ap.add_argument(
        "--stack",
        choices=["h", "v"],
        required=True,
        nargs=1,
        help='the direction of stack: "h" or "v"',
    )
    ap.add_argument(
        "--keep_same_filename",
        action="store_true",
        help="whether use the original filename for the generated",
    )
