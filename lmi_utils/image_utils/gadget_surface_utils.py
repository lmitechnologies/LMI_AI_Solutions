...
class GadgetSurfaceUtils():
    def pkl_2_npy(self, source_path, destination_path, intensity=False):
        ...
        for file in files:
            ...
            if intensity:
                try:
                    intensity = content["intensity_array"]
                    intensity = intensity.view(numpy.uint8) + numpy.uint8(255)

                    numpy.save(join(destination_path, file.replace('.gadget3d.pickle', '-intensity.npy')), intensity)
                except KeyError:
                    continue
