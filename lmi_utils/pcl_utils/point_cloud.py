...
class PointCloud:
    ...
    def resample_cloud(self, dx, dy):
        ...
        grid_z = griddata(
            np.vstack((yinput, xinput)).transpose(),
            zinput,
            (grid_y, grid_x),
            method="nearest",
            fill_value=0.0,
    ...
    def __make_maps(self):
        ...
        self.x_map, self.y_map = dict(zip(x_val, x_pos)), dict(zip(y_val, y_pos))
    ...
    def pad(self, height, width):
        ...
        if height > H:
            ...
            self.img = cv2.copyMakeBorder(self.img, pad_top, pad_bottom, 0, 0, cv2.BORDER_CONSTANT, value=BLACK)
        elif height < H:
            raise Exception(f"Converted image height: {H} is greater than output height: {height}.  Increase output height.")

        if width > W:
            delta = width - W
            pad_right = delta // 2
            pad_left = delta - pad_right
            self.img = cv2.copyMakeBorder(self.img, 0, 0, pad_left, pad_right, cv2.BORDER_CONSTANT, value=BLACK)
        elif width < W:
            raise Exception(f"Converted image width: {W} is greater than output width: {width}.  Increase output width.")
