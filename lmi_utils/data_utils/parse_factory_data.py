...
def extract_imgs(
    ...
    if idx1 == -1 or idx2 == -1:
        raise Exception(
            "archive folder naming convention is not expected. Support two formats: PATH/archive-SKU_TIMESTAMP or PATH/archive_TIMESTAMP"
        )
                ...
                if ext == ".zst":
                    ...
                    for tt in ["intensity", "profile"]:
                        ...
                        if write_file:
                            ...
                            os.makedirs(final_out_path, exist_ok=True)
                            cv2.imwrite(
                                os.path.join(final_out_path, name + "." + tt + ".png"),
                                img,
                            )
                    cnt += 1
                elif ext == ".tiff":
...
if __name__ == "__main__":
    ...
    ap.add_argument(
        ...
        "-i",
        required=True,
        help="the input sku path. Could be a single SKU tarfile/untar folder or a folder containing multiple SKUs",
    )
    ...
    ap.add_argument(
        ...
        required=True,
        nargs=1,
        choices=["anomdet", "objdet"],
        help='choose the task: "anomdet" or "objdet"',
    )
    ap.add_argument(
        "--num_imgs",
        "-n",
        default=20,
        type=int,
        help="the number of images kept for training. defaults to 20",
    )
    ap.add_argument(
        "--target_camera",
        "-t",
        default="all",
        help='the target sensor(s) for parsing data. The format could be either "avt_1" or "avt". default to "all"',
    )
    ap.add_argument(
        "--first_dir",
        "-f",
        default="training-data",
        help='the folder name of training images. default to "training-data"',
    )
    ap.add_argument(
        "--last_dir",
        "-l",
        default="untracked-data",
        help='the folder name of images that are NOT used for training. default to "untracked-data"',
    )
