...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_dir",
        required=True,
        help="Source data directory.  All training data will be moved/copied to data_dir/training dir.",
    )
    parser.add_argument("--training_size", type=int, default=0, help="Training data size.")
    parser.add_argument(
        "--make_test_dir",
        action="store_true",
        help="Set to create a test directory. All non-training files will be moved/copied to data_dir/test dir.  Default to no test dir.",
    )
    parser.add_argument(
        "--make_random",
        action="store_true",
        help="Randomize training data. Default to no randomizing.",
    )
    parser.add_argument(
        "--convert_to_png",
        action="store_true",
        help="Convert imags to .png. Default to not image recasting.",
    )
    parser.add_argument(
        "--rotate_png_90",
        action="store_true",
        help="Rotate images 90 degrees. Default to no rotation.",
    )
    parser.add_argument(
        "--move_files",
        action="store_true",
        help="Move files deletes original files. Default to copy.",
    )
